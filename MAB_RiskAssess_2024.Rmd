...
Low predation threshold: 10%?
More weight on trends–primary consideration, proportion consideration for final bin
Reword the criteria to lead with trend, bin by observed mort proportion for MAFMC managed species, where low = lowest 2, high= highest 2, and the rest in between 

More work!

What percentage of mortality is coming from, e.g. seabirds? How could we handle that?

Sarah - Could add as much information as possible into the food web models and look at where things shake out. Include AT LEAST 50% of predation mortality with selected predators
...
**Risk criteria:**
  ...
  *Risk Level*    *Definition*
  ...
  Low-Moderate    Decreasing overlap/consumption/PPI/mortality trend, 
...
report (Fig. \ref{fig:foragebio}). Indicators of benthic forage will be available for 2025. Food habits data from surveys and literature could be used to
...
The EOP prefers to use species specific habitat indicators from ESPs to start. Maybe model as a backup but not trusted that it captures habitat sufficiently for each species.
...
There is both a significant recent decrease in revenue and a long term significant decrease in gross revenue, indicating high risk to commercial fishery value.  
...
```{r, fig.width = 8, fig.asp = 0.3, fig.cap="Shoreside support businesses"}
...
Newly available time series indicators (CCCVIs) can be added to evaluate change in climate-related risk to community commercial fishery revenue over time from temperature, ocean acidification, stock status, with overall rankings of sensitivity and vulnerability (Figs. \ref{fig.commvulmap}, \ref{fig.commvulex}). Based on these indicators, a high proportion of Mid-Atlantic communities face high climate related risk to commercial revenue. 


```{r commvulmap, fig.cap="Current 5 year average climate vulnerability for Mid-Atlantic community revenue."}
knitr::include_graphics("CCCVI-NE/NE_Total_Vulnerability_092624 - Tarsila Seara - NOAA Federal.jpg")
```

```{r commvulex, fig.asp=.5, fig.cap="Proportion of Mid-Atlantic communities at each revenue climate vulnerability level over time."}
...
# NEW FILE 
magick::image_read("https://github.com/NOAA-EDAB/ecodata/raw/master/workshop/images/NARW_hotspots_final_2024.jpg")
...
```{r foragewind, out.width='75%', fig.cap="Mean occupancy habitats at the 20th (light blue) and 80th (dark blue) quantile thresholds across forage species; gray shows the model extent. Taxa with autumn models include (D) Round Herring, (E) longfin inshore squid, (F) Atlantic Chub Mackerel, (G) Spanish Sardine, (H) Butterfish, and (I) Atlantic Thread Herring."}
