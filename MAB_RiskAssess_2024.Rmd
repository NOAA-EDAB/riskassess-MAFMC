...
The criteria evaluate condition status and major prey trends over the most recent 10 years. The EOP and AP recommended evaluating an alternative calculation using the full time series with a decay factor to emphasize the most recent 10 years. This second alternative will be presented in April 2025 along with the approach presented here. 
...
Indicators for the remaining species will be developed further for the 2025 Risk Assessment.
...
The EOP prefers to use species specific habitat indicators from ESPs to start. 
...
To be developed for each species as ESPs come online.
...
harvest:catch ratio by mode indicator be examined, but not included in the risk criteria. 
...
The EOP Committee and AP also recommended a harvest:catch ratio by mode indicator. The
intent of this indicator would be to evaluate if recreational fishing behavior/preferences are
changing (i.e., harvest versus catch and release) within the different recreational modes/sectors.
There has been a substantial increase in discards as a proportion of catch across all modes (Fig. \ref{fig:discratio}). However, it is unclear from these indicators what is driving this result (e.g. changes in
preferences vs. regulatory discards), or exactly how this would be combined with the effort
diversity indicator to develop a composite measure of risk.
...
At present, the EOP recommends that criteria are based only on the existing diversity indicator.
...
of environmental justice (poverty, population composition, and personal disruptuon), gentrification pressure (housing disruption, retiree migration, and urban sprawl), and economic (labor force structure and housing characteristics) vulnerability, along with commercial and recreational fishing dependence (with dependence defined as medium high or high reliance or engagement). High vulnerability within each category was defined as a medium-high or high score on at least one of the indicators within each category.

In past assessments we used static indicators of sea level rise risk, species
vulnerability to climate change, and catch composition diversity [@colburn_indicators_2016; @jepson_development_2013, @hare_vulnerability_2016]. In the current assessment we have time series indicators of community climate vulnerability.
...
We considered a majority (2 or more out 3) to represent high risk to a
community overall.
...
socrisk <- data.frame(Community = c("Commercial Fishing Dependent", "Recreational Fishing Dependent"),
...
```{r commvulmap, out.width='60%', fig.cap="Current 5 year average climate vulnerability for Mid-Atlantic community revenue."}
...
In past risk assessments based on 2016 data, four of the top communities (20%) had three or more high risk rankings out of five indicators, so we ranked overall social-cultural risk as low-moderate for these Mid-Atlantic communities. However, newer analyses evaluating EJ vulnerability could be incorporated into this analysis.

The current risk assessment uses 2022 data, modified criteria for fewer indicator categories and a different set of social indicators. Over 48% of commercial fishing dependent communities and 34% of recreational fishing dependent communities had two or more high social vulnerability ratings. This would rank communities moderate-high to high for social vulnerability for either sector. Therefore, social vulnerability risk is ranked moderate-high. 

In addition, there is evidence that a majority of Mid-Atlantic communities (including the subset identified as fishery dependent) have high to very high revenue climate vulnerability. This information can be specifically included in future assessments with additional criteria. However, it reinforces at least a moderate-high risk ranking for social vulnerability. 
...
managed fisheries from the NEFSC bottom trawl survey from two different time periods were
used (2014-2018 and 2019-2023). These time periods were selected based on feedback by the
Committee and AP in July that recommended using shorter time blocks and the most recent data
...
of positive catches for each species inside each WEA during the two time blocks. Also, as
recommended by the Committee and AP in July, a weighted spatial overlap was calculated based
on the current development stage of each WEA - planning, construction, or operational. WEAs
...
The total spatial overlap across all WEA’s and a total weighted spatial overlap for the 2014-2018
and 2019-2023 time blocks for summer flounder are provided below.
...
The proposed risk criteria are similar to those presented to the Committee and AP back in July
except under the high-risk definition the term “significant increases” was replaced with “large
increases”. This change was recommended by the Committee and AP since “significant” has a
statistical meaning and is not being applied here. In addition, the low-moderate definition now
includes “decreasing or similar” overlap to account for situations where there is overlap with a
WEA and stock/habitat distribution but the trend is neither increasing or decreasing. 

**Risk Assessment**

Using the proposed risk criteria, summer flounder, scup, black sea bass, Atlatic mackerel, butterfish, longfin squid, bluefish, and spiny dogfish are assigned low-moderate risk. Summer flounder had declining overlap during 14-18 and 19-23 time periods and similar wieghted overlap between two time periods. Scup had declining overlap during 14-18 and 19-23 time periods and declining wieghted overlap between two time periods. Black sea bass had slightly declining overlap during 14-18 and 19-23 time periods and similarly small declines in wieghted overlap between two time periods. Atlantic mackerel had similar spatial overlap during 14-18 and 19-23 time periods and declining wieghted spatial overlap between two time periods. Butterfish had decreasing spatial overlaop during the 14-18 and 19-23 time period and stable/similar weighted overlap during two time periods. Longfin squid had declining spatial overlap during 14-18 and 19-23 time periods and slight increases in weighted spatial overlap between two time periods. Bluefish had moderate decreases in spatial and weighted spatial overlap between 14-18 and 19-23 time periods. Spiny dogfish had slight increases in spatial and weighted spatial overlap between 14-18 and 19-23 time periods.  

Shortfin squid, golden tilefish, blueline tilefish, and chub mackerel are assigned low risk. Only 2 Illex squid and 1 chub mackerel were caught in WEAs during two time periods considered (14-18 and 19-23). No golden or blueline tilefish were caught in WEA during two time periods considered (14-18 and 19-23).  This risk factor is not relevant for deepsea corals. 

Since the stock/habitat distribution indicator is developed at the stock level, the same risk level is assigned the same for both the commercial and recreational sectors.
...
What: Amount of revenue within and relevant survey overlap with WEAs
Who: Species specific revenue estimates from NMFS; NEFSC bottom trawl strata and catch by species
How: Proportion of revenue in WEA relative to total fishery revenue; proportion of NEFSC trawl survey strata by species within WEA
...
**Potential risk criteria:**
  ...
  *Risk Level*    *Definition*
  ...
  Low-Moderate    Low revenue in WEA (</=5%); low survey overlap with WEAs (</=10%)

  Moderate-High   Moderate revenue in WEAs (6-19%); moderate survey overlap with WEAs (11-30%)

  High            Significant revenue in WEAs (>20%); significant survey overlap with WEA (>30%)

  -----------------------------------------------------------------------

**Risk Assessment**

A subset of species have been assessed, with more to be added as the data become available. Commercial summer flounder, scup, Atlantic mackerel, butterfish, longfin squid, bluefish, and chub mackerel have low-moderate risk. Commercial black sea bass has moderate-high risk.  
...
displace fishing activities (Figure 8).

The spatial overlap was then calculated by taking the total area of the non-fishing activities and
amount of estimated habitat within the non-fishing areas. An example “Other Ocean Activities”
spatial overlap map and calculations for summer flounder are provided here.
...
With a quantitative evaluation available for this indicator, there is the potential to use a
range/binned approach to specify risk level (e.g., 0-10% overlap, low risk; 11-25% overlap, lowmoderate risk; 26-50% overlap, moderate-high risk; >50% overlap, high risk), but the bins
associated with the risk level would likely be arbitrary. Similarly, without some specificity to the
risk criteria definition, defining what constitutes “low-moderate”, “moderate”, and “high” risk
would also be arbitrary. 

**Risk Assessment**

Using the potential risk criteria definitions and/or the example binned levels, the example for
summer flounder would be assigned low risk. While summer flounder habitat does have
moderate overlap with shipping lanes, there is minimal, near zero overlap with the currently
included monuments, sanctuaries, or protected areas. Since the habitat distribution indicator is
developed at the stock level, the same risk level would be assigned the same for both the
commercial and recreational summer flounder sectors.

The risk assessment can be completed for the other stocks which have the NRHA habitat distribution model results
available (bluefish, butterfish, Illex squid). 
