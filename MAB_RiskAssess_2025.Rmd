...
```{r mako, fig.asp = .4, fig.cap="Mako shark B/Bmsy. Dashed line depicts where B is equal to BMSY. Trend line depicts statistically significant trend", echo=FALSE}
...
Mean winter bottom temperature was explored as an indicator of overwinter survival. Recruitment of Black sea bass (BSB) has been shown to be higher than average in warm years and below average in cold years [@tabandera_working_2024]. Additionally, BSB have a preferred temperature of >8 C [@miller_offshore_2016]. A mean winter (February and March) bottom temperature indicator was developed using observed ocean temperature data from the [Hydrographic Database](https://comet.nefsc.noaa.gov/erddap/info/ocdbs_v_erddap1/index.html) maintained by the NEFSC Oceans Branch. These data have been collected from ECOMON and bottom trawl survey vessels sampling throughout the Northeast U.S. shelf. Sample locations numbered in the dozens for each month (February and March) in both the Northern and Southern MAB for each year in the time series. Area weighted regional mean values of bottom temperature were calculated as in [@fratantoni_description_2013]. As noted in the ESP [@tabandera_working_2024], winter conditions were represented by averaged February-March data due to low sampling coverage in January.
...
Habitat condition indicators assessed the cold pool index, cold pool spatial extent index, cold pool persistence index, and the mean bottom temperature and salinity of the spatial extent in which Golden tilefish have been captured by the NMFS bottom trawl survey. The bottom temperature and salinity optimal ranges for golden tilefish are 9-14 °C [@grimes_tilefish_1986], and 33-36 PSU [@steimle_essential_1999], respectively. Offshore habitat with temperature and salinity outside optimal ranges could affect golden tilefish spawning and recruitment. If the cold pool has a larger extent or persists for a longer time, environmental conditions are not optimal for early life history stages and recruitment success. The cold pool index indicators are generated from bias-corrected ROMS-NWA and GLORYS data. Bottom temperature and salinity indicators were generated using [GLORYS12](https://data.marine.copernicus.eu/product/GLOBAL_MULTIYEAR_PHY_001_030/description) [@eu_copernicus_marine_service_information_global_2023], a data-integrated ocean model.
...
Physical oceanography indicators assessed the sea surface temperature (SST), gulf stream index (GSI), and shelf water volume, temperature, and salinity. SST was used as an indicator of water mass movement [@fisher_life_2014], which could negatively impact eggs and larvae through displacement. SST was derived from the Advanced Clear Sky Processor for Ocean (ACSPO) system. A positive GSI indicates a more northerly Gulf Stream position, which pushes warm water onto the shelf, increasing the amount of suitable habitat for Golden tilefish. The GSI is calculated using observations of [temperature at depth](https://www.metoffice.gov.uk/hadobs/en4/) using methods described in ecodata indicator documentation^[https://noaa-edab.github.io/catalog/gsi.html]. Shelf water volume, temperature, and salinity were derived from the Hydrographic Database maintained by the NEFSC Oceans and Climate Branch [@fratantoni_description_2013]. Shelf water physical oceanography indicators were compare to tolerance thresholds from the literature to assess changes in habitat suitability.
...
```{r gsi, fig.cap="Gulf stream index. Dashed line depicts time series mean.", echo=FALSE}
...
construction. \textcolor{red}{However, recent policy changes and priorities at the state and federal level have made the timing and pace of offshore wind development much more uncertain, particularly in the near term.} Offshore wind siting, construction, and operation has the potential for a variety of
...
operations, and revenue are anticipated to change. \textcolor{red}{However, recent policy changes and priorities at the state and federal level have made the timing and pace of offshore wind development much more uncertain, particularly in the near term.} 
