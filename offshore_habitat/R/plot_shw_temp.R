#' plot mean shelf water temperature
#'
#'
...
plot_shw_temp <- function(shadedRegion=NULL,
                         report = "MidAtlantic") {
  ...
  shw_temp_mean <-  var.means |>
    dplyr::filter(Var == 'shw.t') |>
    dplyr::pull(mean)
  ...
  p <- shw_vol_temp_sal |>
    ...
    ggplot2::geom_line() +
    ggplot2::geom_point()  +
    ggplot2::geom_hline(yintercept=shw_temp_mean,linetype=setup$hline.lty)+
    ggplot2::facet_wrap(~EPU) +
    ggplot2::ylab("Temperature (C)") +
    ggplot2::xlab(ggplot2::element_blank())+
    ggplot2::ggtitle(paste(report,": mean shelf water temperature")) +
    ecodata::geom_gls() +
    ecodata::theme_ts()+
    ecodata::theme_facet()+
    ecodata::theme_title()
